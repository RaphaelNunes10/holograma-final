		...
		public override void Process (LipSyncData inputClip, AutoSync.ASProcessDelegate callback)
		{
			...
			if (audioPath != null)
			{
				...
				if (AutoSyncConversionUtility.IsConversionAvailable && useAudioConversion)
				{
					...
					if (!AutoSyncConversionUtility.StartConversion(audioPath, newPath, AutoSyncConversionUtility.AudioFormat.WavPCM, 16000, 16, 1, AutoSyncConversionUtility.EncodingType.UnsignedInteger, AutoSyncConversionUtility.Endianness.LittleEndian))
						failed = true;
					audioPath = newPath;
				}
