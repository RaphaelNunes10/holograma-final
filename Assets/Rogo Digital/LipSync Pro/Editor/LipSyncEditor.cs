				...
				for (int e = 0; e < lsTarget.emotions.Count; e++)
				{
					...
					for (int b = 0; b < lsTarget.emotions[e].bones.Count; b++)
					{
						...
						if (savingRelative)
						{
							...
							bone.localRotation = lsTarget.emotions[e].bones[b].endRotation - lsTarget.emotions[e].bones[b].neutralRotation;
						}
						else
						{
				...
				for (int shape = 0; shape < preset.emotionShapes.Length; shape++)
				{
					...
					for (int bone = 0; bone < preset.emotionShapes[shape].bones.Length; bone++)
					{
						...
						if (b)
						{
							...
							if (preset.isRelative)
							{
								newBone.endPosition = newBone.neutralPosition + preset.emotionShapes[shape].bones[bone].localPosition;
								newBone.endRotation = newBone.neutralRotation + preset.emotionShapes[shape].bones[bone].localRotation;
							}
							else
							{
